        ...
        public static void init(MustacheTemplateEngine engine) {
                ...
                get("/admin/subjects", (req, res) -> {
                        ...
                        if (!isLoggedIn(req)) {
                                ...
                                logger.warning("Intento de acceso sin sesión al listado de materias.");
                        ...
                        model.put("subjectCount", subjectService.getTotalSubjectsCount());

                        return engine.render(new ModelAndView(model, "subjects.mustache"));
                });
                ...
                get("/admin/subjects/new", (req, res) -> {
                        ...
                        if (!isLoggedIn(req)) {
                                ...
                                logger.warning("Intento de acceso sin sesión al formulario de materia.");
                        ...
                        logger.info("Renderizando formulario de creación de materia.");

                        Map<String, Object> model = new HashMap<>();

                        model.put("username", req.session().attribute("username"));

                        return engine.render(new ModelAndView(model, "subject_new.mustache"));
                });
                ...
                post("/admin/subjects/new", (req, res) -> {
                        ...
                        if (!isLoggedIn(req)) {
                                ...
                                logger.warning("Intento de creación de materia sin sesión.");
                        ...
                        try {
                                ...
                                Integer weeklyHours =
                                                Integer.parseInt(req.queryParams("weekly_hours"));

                                String modality = req.queryParams("modality");

                                Long studyPlanId = null;

                                subjectService.createSubject(name, code, weeklyHours, modality,
                                                studyPlanId);

                                logger.info("Materia creada correctamente.");

                                res.redirect("/dashboard");

                        } catch (IllegalArgumentException e) {

                                logger.warning("Error de validación al crear materia: "
                                                + e.getMessage());
                                ...
                                return engine.render(
                                                new ModelAndView(model, "subject_new.mustache"));

                        } catch (Exception e) {

                                logger.severe("Error inesperado al crear materia: "
                                                + e.getMessage());

                                res.redirect("/error?type=DatabaseError&message=No se pudo crear la materia.");
                        }
                ...
                post("/admin/subjects/:id/correlativities", (req, res) -> {
                        ...
                        Long requiredSubjectId =
                                        Long.parseLong(req.queryParams("required_subject_id"));

                        boolean requiresApproved = req.queryParams("requires_approved") != null;

                        correlativityService.addCorrelativity(subjectId, requiredSubjectId,
                                        requiresApproved);
